...
class CheckPlayersCommandTask(BaseTask):
   def __init__(self, discord_client, setting_manager, players_check, last_messages):
   ...
   async def run(self, server_id, time, channel):
      ...
      users_without_character = await self.players_check.check(server_id, [
         {'id': settings[SettingName.PLAYER_WITHOUT_CHARACTER_ROLE_ID.value], 'channels': [settings[SettingName.CHARACTERS_CHANNEL_ID.value]]}
      ], PlayerCheckMethod.JOIN_DATE, 2)
      message = "Players without characters (from 2 days):\n"
      for user in users_without_character:
         last_message = None
         if details_mode:
            last_message = await self.last_messages.find_message_by_user(user, settings[SettingName.OFFTOPIC_CATEGORY.value], GetLastMessageMode.CATEGORIES)
         if last_message is not None:
            message = message + "\n- <@{0}> (last message {1} days ago on <#{2}>)".format(user.id, (datetime.datetime.now() - last_message.created_at).days, last_message.channel.id)
         else:
            message = message + "\n- <@{0}>".format(user.id)
      categories_to_read = settings[SettingName.CATEGORY_FOR_LOOKING_PLAYERS.value]
      channels_to_read = []
      ...
      inactive_users = await self.players_check.check(server_id, [
         {'id': settings[SettingName.PLAYER_WITH_CHARACTER_ROLE_ID.value], 'channels': channels_to_read}
