...
if __name__ == "__main__":
    ...
    check_players_task = CheckPlayersTask(
        client, setting_manager, players_check, database_manager)
    stats_command_task = StatsCommandTask(client, setting_manager)
    check_players_command_task = CheckPlayersCommandTask(
        client, setting_manager, players_check, last_messages)
    talk_command = TalkCommand(client, mention, permission_checker)
    check_command = CheckCommand(
        client, mention, permission_checker, check_players_command_task)
    stats_command = StatsCommand(
        client, mention, permission_checker, stats_command_task)
    commands = {
        "talk": talk_command,
        "check": check_command,
        "stats": stats_command
