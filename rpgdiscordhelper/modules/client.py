...
class ExDiscordClient(DiscordClientBase):
    ...
    async def on_message(self, message):
        if message is not None and message.guild is not None:
            ...
            if len(message.content) <= 0 or message.author == self.user:
                return
            if str(message.channel.category.id) in (
                    settings[SettingName.CATEGORY_FOR_LOOKING_PLAYERS.value]):
                session = self.database_manager.create_session()
                session.query(InactiveUser).filter(
                    InactiveUser.user_id == str(message.author.id),
                    InactiveUser.server_id == str(message.guild.id)
                    ).update({'status': 0}, synchronize_session=False)
                session.commit()

            if message.content[0] == '/':
                data = self.arg_parser.parse(message.content)
                await self.command_executor.execute(
                    data[0], message.author, message.channel, data[1:])
    ...
    async def on_member_update(self, before, after):
        ...
        if player_with_character_role_id not in (
            ...
            session.query(UserWithoutCharacter).filter(
                UserWithoutCharacter.user_id == str(after.id),
                UserWithoutCharacter.server_id == str(after.guild.id)
                ).update({'status': 0}, synchronize_session=False)
            session.commit()
    ...
    async def on_member_remove(self, member):
        session = self.database_manager.create_session()
        session.query(UserWithoutCharacter).filter(
            UserWithoutCharacter.user_id == str(member.id),
            UserWithoutCharacter.server_id == str(member.guild.id)
            ).update({'status': 0}, synchronize_session=False)
        session.query(InactiveUser).filter(
            InactiveUser.user_id == str(member.id),
            InactiveUser.server_id == str(member.guild.id)
            ).update({'status': 0}, synchronize_session=False)
        session.commit()
