            ...
            for channels_ids_to_check in [d['channels'] for d in dict_data]:
                for channel_id in channels_ids_to_check:
                    channel = discord.utils.find(
                        lambda c: c.id == channel_id, guild.channels)
                    async for m in channel.history(limit=200).filter(
                            lambda msg: msg.created_at + datetime.timedelta(
                                days=arg_for_mode) > datetime.datetime.now()):
                ...
                if role_id in roles_ids_to_check:
                    if mode is PlayerCheckMethod.JOIN_DATE:
                        if datetime.datetime.now() > (
                                member.joined_at +
                                datetime.timedelta(days=arg_for_mode)):
                        ...
                        if datetime.datetime.now() > (
                            ...
                            message = discord.utils.find(
                                lambda m: m.author == member, loaded_messages)
                            if message is None:
                                founded_user_names.append(member)
